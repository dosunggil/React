/*
  ...
  Component 가 rendering 될 때 자동으로 코드를 실행하여
...
/*
  ...
  List 를 보여주고 다음에 item 을 클릭하거나 변경하는 코드를 추가하려면 어디에서 state 데이터를 만들어야 할 것인가를 잘 고민해야 한다.
  DietMain 에서 DB 를 fetch 하고 item 등을 변화시키는 event 도 선언하여
  Main Component 가 감싸고 있는 child Comps 들이 공유할 수 있도록 설계를 한다.
*/
const DietMain = () => {
  // useState 와 useEffect 는 함수 내부에서 실행될 코드
  const [foods, setFoods] = useState([]);

  // 데이터를 fetch 할 때 사용할 함수 선언
  ...
  // DietMain 컴포넌트가 rendering 될 때 실행하여 fetchFood 함수를 호출하고 결과를 foods state 변수에 세팅하기
  // useEffect 의 두번째 매개변수
  // 두 번째 매개변수에 state 변수를 설정하면 state 변수값이 변화될때마다 useEffect 함수가 실행된다.
  // state 변수의 변화를 감지하여 어떤 event 를 실행할 때 사용하는 방법

  // 두번째 매개변수에 빈(blank) 배열([]) 을 설정하면
  // 컴포넌트가 rendering 될 때 한번만 실행하도록 하는 방법
  // 두번째 매개변수를 생락하게 되면 useEffect 함수가
  ...
  */
  ...
  /*
    child component 에 전달해야 할 데이터와 함수가 많아지는 경우
  ...
  */
  ...
  // return (
  //   <>
  //     <DietList removeFoodItem={removeFoodItem} foods={foods} />
