/*
  ...
  Hook 함수라고 한다.
  component 의 lifecycle 과 관련된 함수들이다.
  state 를 만들거나, state 변화를 감시하거나
  component 가 rendering 되는 시점에 무언가를 실행시키거나 하는 용도의 함수들이다.
*/
...
const BucketMain = () => {
  ...
  */
  ...
  /*
    ...
    localStorage 에 BUCKETLIS 라는 이름으로
  ...
  */
  ...
  // 버킷을 입력하고 enter 를 누르면 버킷이 추가되는 함수
  const onKeyDown = (e) => {
    if (e.keyCode === 13) {
      const bucket = {
        b_id: uuid(), // uuid 를 사용하여 PK 생성
        b_flag: 0, // 버킷의 중요도를 표시하는 데이터
  ...
  const bucket_delete = (id) => {
    ...
    setBuckList(bucketRemoveList);
  }; // end bucket_delete

  // 중요도 칼럼을 클릭했을 때 b_flag 변수값을 변경하는 함수
  const bucket_flag_toggle = (id) => {
    ...
    const bucketBody = bucketList.map((bucket) => {
      if (bucket.b_id === id) {
        /*
          원래의 bucket 데이터를 spread 하고
          b_flag 값만 원래의 값에 1을 더하여 변경하고
  ...
  return (
    <>
      <div>
        <input
          onKeyDown={onKeyDown}
          placeholder="추가할내용을입력하세요"
          className="w3-input w3-border w3-hover-gray w3-round"
        />
      </div>
      <div>
        <BucketList bucketList={bucketList} fucntions={fucntions} />
      </div>
    </>
  );
};
