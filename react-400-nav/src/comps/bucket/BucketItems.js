...
function BucketItems(props) {
  ...
  /*
    ...
    자식 comp 는 props 를 통해 데이터를 받아 표출한다.(화면에 보여준다.)
    
    이때 만약 자식 comp 에서 부모 comp 가 생성 선언한 state 변수를 변경해야 할 경우가 발생하면,
    부모 comp 에서 state 변수를 변경하는 함수를 함께 만들어
    자식 comp 에게 전달해야 한다.

    지금 <td onClikc = {()=> bucket_complete(id)}> 의 코드를 보면
    현재 화면(comp) 에 선언된 td 가 클릭되었을 때 bucket_complet 함수를
  ...
  */
  ...
  return (
    <tr className={item.b_end_date ? "cancle" : ""}>
...
/*
  event 핸들러에게 어떤 값을 전달하면서 실행하고자 할 때 다음과 같은 코드는 절대 사용하면 안된다.
  <td onClick={bucket_complete(item.b_id)}></td>
  
  위의 코드는 event(click) 과 상관 없이 화면이 rendering 될 때 내부의 함수가 실행되어 버린다.
