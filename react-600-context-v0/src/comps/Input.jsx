...
const Input = (props) => {
  ...
  /*
    ...
    만약 화면의 inout box 에 키보드로 문자열을 입력하면
    value 속에 연결된 변수(state, 일반)의 값을 직접 변경하는 성질이 발생한다.
    react 에서는 state 변수를 임의로 변경하는 것이 금지되어있다.
    이러한 특성을 유지하기 위하여 value={변수} 형태로 할당이 되면
    쓰기금지(read only) 성질이 발생한다.

    이럴때 defaultValue 를 사용하여 변수를 할당할 수  있지만
    간혹 defaultValue 로 설정된 변수값은 state의 특성을 제대로 발휘하지 못한다.
    그래서 defaultValue 보다는 value 를 사용한다.
  ...
  */
  const onChangeEvent = (e) => {
    ...
    const name ="a_tel" 이라는 변수가 있을 대
    [name] 형식으로 사용하면 마치 a_tel 이라는 변수가 만들어지는 것처럼 사용할 수 있다.
    event 핸들러의 e.target.name 값을 사용하여 
  ...
  /*
    저장 버튼을 클릭하면
    address 에 저장된 데이터를 addrList 에 추가하기
    ...
    setAddrList 함수를 사용하여 addrList 에 추가하는 코드를 작성하겠다.
  */
